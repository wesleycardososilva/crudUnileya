...
export default function FormLivro() {
  ...
  const onFinish = (values) => {
    if (id != 0) {
      LivrosService.update(id, idAutor, idGenero, idEditora, values.titulo, values.ano_de_lancamento)
        .then((response) => {
        ...
        LivrosService.add(idAutor, idGenero, idEditora, values.titulo, values.ano_de_lancamento)
        .then((response) => {
          window.location.href = "/livros";
        })
        .catch((error) => {
      ...
      <Form.Item
        ...
        >
          <Select
            ...
            filterOption={(input, option) =>
              option.childrenEditora.toLowerCase().indexOf(input.toLowerCase()) >= 0
            }
          >

            {childrenEditora && childrenEditora.map((element)=>{
        ...
        >
          <Select
            ...
            onSearch={onSearch}
            filterOption={(input, option) =>
              option.childrenGenero.toLowerCase().indexOf(input.toLowerCase()) >= 0
            }
          >

            {childrenGenero && childrenGenero.map((element)=>{
                return <Option key={element.id }>{element.nome}</Option>;
            })}
          </Select>
        </Form.Item>
        ...
        >
          <Select
            ...
            onSearch={onSearch}
            filterOption={(input, option) =>
              option.childrenAutor.toLowerCase().indexOf(input.toLowerCase()) >= 0
            }
          >

            {childrenAutor && childrenAutor.map((element)=>{
                return <Option key={element.id }>{element.nome}</Option>;
            })}
          </Select>
        </Form.Item>
